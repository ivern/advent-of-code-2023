...
public class Puzzle14 {
    ...
    private static final Encoder ENCODER = new Encoder("J23456789TQKA");
    ...
    int getHandType(String hand) {
        var counter = Counter.countCharacters(hand);

        int jokers = counter.get('J');
        counter.remove('J');
        var counts = counter.descending();

        if (counts.size() == 1 || counts.isEmpty()) {
            return 7;
        } else if (counts.size() == 2) {
            return (counts.get(0).getValue() + jokers == 4) ? 6 : 5;
        } else if (counts.size() == 3) {
            return (counts.get(0).getValue() + jokers == 3) ? 4 : 3;
